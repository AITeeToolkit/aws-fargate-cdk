...
def list_indices(
    endpoint: str, auth: AWS4Auth, exclude_system: bool = True
) -> List[str]:
...
def create_index_with_mapping(
    endpoint: str, index: str, mapping: Dict[str, Any], auth: AWS4Auth
):
...
def backup_index(
    ...
    target_endpoint: str,
    index: str,
    source_auth: AWS4Auth,
    target_auth: AWS4Auth,
    batch_size: int = 1000,
):
    ...
    scroll_body = {"size": batch_size, "query": {"match_all": {}}}
    ...
    while True:
        ...
        for hit in hits:
            # Index action
            bulk_data.append(
                json.dumps({"index": {"_index": index, "_id": hit["_id"]}})
            )
            # Document source
        ...
        response = requests.post(
            bulk_url, auth=target_auth, data=bulk_body, headers=headers
        )
        response.raise_for_status()
        ...
        scroll_continue_body = {"scroll": "5m", "scroll_id": scroll_id}

        response = requests.post(
            scroll_continue_url, auth=source_auth, json=scroll_continue_body
        )
        response.raise_for_status()
        scroll_data = response.json()
        scroll_id = scroll_data.get("_scroll_id")
...
def main():
    parser = argparse.ArgumentParser(
        description="Backup OpenSearch indices from one domain to another"
    )
    ...
    parser.add_argument(
        "--indices", default="*", help="Comma-separated list of indices or * for all"
    )
    parser.add_argument("--region", default="us-east-1", help="AWS region")
    parser.add_argument(
        "--batch-size", type=int, default=1000, help="Batch size for scroll"
    )
