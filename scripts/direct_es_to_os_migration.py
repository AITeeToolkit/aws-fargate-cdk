...
def get_elasticsearch_data_from_k8s():
    ...
    try:
        response = requests.get(
            f"{es_endpoint}/_cluster/health", timeout=5, verify=False, auth=auth
        )
        ...
        print(
            "💡 Make sure port forwarding is active: kubectl port-forward -n elasticsearch svc/elasticsearch-master 9200:9200"
        )
        return None
    ...
    try:
        ...
        for index_info in data_indices:
            ...
            mapping_response = requests.get(
                f"{es_endpoint}/{index_name}/_mapping", verify=False, auth=auth
            )
            if mapping_response.status_code == 200:
                mapping = mapping_response.json()
                exported_data.append(
                    {"action": "create_index", "index": index_name, "mapping": mapping}
                )

            # Export documents using scroll API
            scroll_response = requests.post(
                f"{es_endpoint}/{index_name}/_search?scroll=5m",
    ...
    try:
        ...
        for item in data:
            if item["action"] == "create_index" and item["index"] not in indices_created:
                ...
                create_request = AWSRequest(
                    method="PUT", url=create_url, data=json.dumps(item["mapping"])
                )
        ...
        for item in data:
            if item["action"] == "index":
                # Add to bulk request
                bulk_data.append(
                    json.dumps({"index": {"_index": item["index"], "_id": item["id"]}})
                )
